...
<div class="page-head-row" style="margin-bottom:8px;">
    <div>
        <h2 class="page-head-title" style="font-size: 20px;">Edit Produk: {{ $barang->NamaBrg }}</h2>
        <p class="page-head-sub">Ubah data produk · Tekan Enter untuk pindah ke field berikutnya</p>
    </div>
    <a href="{{ url('/admin/barang') }}" class="btn-add-blue" style="padding: 6px 14px; font-size: 13.5px;">← Kembali ke Daftar</a>
</div>

@if (session('info'))
    <div style="background: #FEF3C7; border: 1.5px solid #F59E0B; color: #92400E; padding: 8px 14px; border-radius: 8px; margin-bottom: 8px; font-weight: 600; font-size: 14px; display: flex; align-items: center; gap: 8px;">
        ℹ️ <span>{{ session('info') }}</span>
    </div>
@endif
    ...
    <form id="formEditBarang" method="POST" action="{{ url('/admin/barang/update/' . $barang->KodeBrg) }}">
        ...
        <div class="form-action-row" style="margin-top: 20px;">
            <button type="submit" class="btn-simpan-form warning">✏️ Simpan Perubahan (Rubah)</button>
